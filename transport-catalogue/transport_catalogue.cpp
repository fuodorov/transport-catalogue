...
void TransportCatalogue::AddStop(Stop stop) {
  const auto position =
      stops_storage_.insert(stops_storage_.begin(), std::move(stop));
...
void TransportCatalogue::AddDistance(std::string_view stop_from,
  ...
  distances_between_stops_.insert(
...
TransportCatalogue::GetBusStatistics(std::string_view bus_number) const {
  ...
  result.curvature = static_cast<double>(result.rout_length) /
                     CalculateGeographicLength(bus_info);

  return result;
}

int TransportCatalogue::CalculateRouteLength(
...
void TransportCatalogue::UpdateMinMaxStopCoordinates(
...
TransportCatalogue::GetOrderedBusList() const {
  return ordered_bus_list_;
}

BusStopsStorage
TransportCatalogue::GetFinalStops(std::string_view bus_name) const {
...
BusStopsStorage
TransportCatalogue::GetRouteInfo(std::string_view bus_name,
...
StringViewPairStorage<Info>
TransportCatalogue::GetAllDistancesOnTheRoute(std::string_view bus_number,
...
TransportCatalogue::GetBusesPassingThroughTheStop(
