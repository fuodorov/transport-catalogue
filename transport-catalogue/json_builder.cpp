...
StartContainersContext::StartContainersContext(Builder &builder)
    : BaseContext(builder) {}
...
Builder::Builder()
    : KeyContext(*this), ValueContext(*this), DictContext(*this),
      ArrayContext(*this) {}
...
Builder &Builder::Value(Node::Value value) {
  ...
  std::visit(
      [this](auto &&v) {
        nodes_stack_.emplace_back(std::make_unique<Node>(v));
      },
      value);
...
void Builder::AddNode(Node top_node) {
  ...
  if (nodes_stack_.empty()) {
    ...
    std::get<Array>(nodes_stack_.back()->GetValue())
        .emplace_back(std::move(top_node));
  } else if (nodes_stack_.back()->IsString()) {
    std::string key = std::get<std::string>(nodes_stack_.back()->GetValue());
    nodes_stack_.pop_back();
    std::get<Dict>(nodes_stack_.back()->GetValue())
        .emplace(std::move(key), std::move(top_node));
  } else {
