...
const std::unordered_map<Stop *, RouterByStop> &TransportRouter::get_stop_to_vertex() const {
    return stop_to_router_;
}
const std::unordered_map<EdgeId, std::variant<StopEdge, BusEdge>> &TransportRouter::get_edge_id_to_edge() const {
...
void TransportRouter::add_edge_to_bus(TransportCatalogue &transport_catalogue) {
    for (auto bus : get_bus_ptr(transport_catalogue)) {
        parse_bus_to_edges(bus->stops.begin(), bus->stops.end(), transport_catalogue, bus);

        if (!bus->is_roundtrip) {
            parse_bus_to_edges(bus->stops.rbegin(), bus->stops.rend(), transport_catalogue, bus);
        }
    }
}
