...
SphereProjector MapRenderer::get_sphere_projector(const std::vector<geo::Coordinates> &points) const {
    return SphereProjector(points.begin(), points.end(), render_settings_.width_, render_settings_.height_,
                           render_settings_.padding_);
}

void MapRenderer::init_sphere_projector(std::vector<geo::Coordinates> points) {
    sphere_projector = SphereProjector(points.begin(), points.end(), render_settings_.width_, render_settings_.height_,
                                       render_settings_.padding_);
}
...
void MapRenderer::set_route_text_common_properties(svg::Text &text, const std::string &name,
...
void MapRenderer::set_route_text_additional_properties(svg::Text &text, const std::string &name,
                                                       svg::Point position) const {
...
void MapRenderer::set_route_text_color_properties(svg::Text &text, const std::string &name, int palette,
                                                  svg::Point position) const {
    set_route_text_common_properties(text, name, position);
...
void MapRenderer::set_stops_text_common_properties(svg::Text &text, const std::string &name,
                                                   svg::Point position) const {
    using namespace std::literals;
...
void MapRenderer::set_stops_text_additional_properties(svg::Text &text, const std::string &name,
                                                       svg::Point position) const {
    using namespace std::literals;
...
void MapRenderer::add_buses_name(std::vector<std::pair<Bus *, int>> &buses_palette) {
    ...
    for (auto [bus, palette] : buses_palette) {
        ...
        if (!bus_empty) {
            if (bus->is_roundtrip) {
                set_route_text_additional_properties(route_name_roundtrip, std::string(bus->name),
                                                     sphere_projector(stops_geo_coords[0]));
                map_svg.Add(route_name_roundtrip);

                set_route_text_color_properties(route_title_roundtrip, std::string(bus->name), palette,
                                                sphere_projector(stops_geo_coords[0]));
                ...
                if (stops_geo_coords[0] != stops_geo_coords[stops_geo_coords.size() / 2]) {
                    ...
                    set_route_text_color_properties(route_title_notroundtrip, std::string(bus->name), palette,
                                                    sphere_projector(stops_geo_coords[stops_geo_coords.size() / 2]));
...
void MapRenderer::add_stops_name(std::vector<Stop *> &stops) {
    ...
    for (Stop *stop_info : stops) {
        if (stop_info) {
            ...
            set_stops_text_additional_properties(svg_stop_name, stop_info->name, sphere_projector(coordinates));
            map_svg.Add(svg_stop_name);

            set_stops_text_color_properties(svg_stop_name_title, stop_info->name, sphere_projector(coordinates));
