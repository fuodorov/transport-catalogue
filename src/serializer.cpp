...
transport_catalogue_protobuf::TransportCatalogue transport_catalogue_serialization(
    ...
    for (const auto &[pair_stops, pair_distance] : distances) {
        ...
        distance_proto.set_start(calculate_id(stops.cbegin(), stops.cend(), pair_stops.first->name));

        distance_proto.set_end(calculate_id(stops.cbegin(), stops.cend(), pair_stops.second->name));
...
domain::RoutingSettings routing_settings_deserialization(
    const transport_catalogue_protobuf::RoutingSettings &routing_settings_proto) {
